...
from .classification_models import load_resnet
...
class DecoderBlock(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=in_ch + skip_ch, out_channels=out_ch, kernel_size=3, stride=1, padding=1,
                               bias=False)
...
class UNet(nn.Module):
    def __init__(
        ...
        if encoder_name in ["resnet18", "resnet34"]:
            ...
            raise ValueError(
                f"No such encoder: {encoder_name}. Try one of ['resnet18', 'resnet34', 'resnet50', 'resnet101']")
