...
**Figure 3**: Pixel-based classification results shown in a normalised confusion matrix in percent. The segmentation model was trained on sharp image patches only. Background is denoted by BG, sorghum by S and weed by W. **a** Results of `WeedSeg` on the full hold-out test-set (top) and separated in sharp (left) and motion blurred (right) dataset. **b** Results of `DeBlurWeedSeg` on the full hold-out test-set (top) and separated in sharp (left) and motion blurred (right) dataset.
